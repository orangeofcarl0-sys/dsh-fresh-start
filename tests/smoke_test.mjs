...
// 用例 1：全流程（总结成功 → 新会话带 seed → 归档）
{
  ...
  check('new session seeded with summary', recorded.created.length === 1 && recorded.created[0].seed?.[1]?.data?.content?.[0]?.text?.includes('SUMMARY CONTENT'))
  check('seed has turn/start (non-blank)', recorded.created[0].seed?.[0]?.type === 'turn/start' && recorded.created[0].seed?.[2]?.type === 'turn/end')
  check('seed summary message has surfaceOp append', recorded.created[0].seed?.[1]?.surfaceOp === 'append')
