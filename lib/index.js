...
export function apply(ctx) {
  ...
  async function executeFresh(invocation) {
    ...
    try {
      ...
      // seed 用一个「已完成的摘要轮」（turn/start → user/message → turn/end）。
      // 必须含 turn/start：否则新会话被 dsh 判定为 blank（sessionBlank 检查是否
      // 存在 turn/start），blank 会话在侧栏只作为「New Session 占位」、不进历史列表。
      ...
      const seed = summary === null ? void 0 : [
        ...
        { type: 'turn/end', seq: 2, time: seedTime + 2, data: { turn: 1, reason: { kind: 'completed' } } },
      ]
